...
The Graph is connected and all nodes can be visited starting
from the given node.
"""

# Key Note:
# 1. use bfs, but the difference from tree is that we need
# to do the connection. so the value of map is not only 
# neighbors, but also the node.
# visited = {
#     node: Node(node.val, neighbors=[])
# }