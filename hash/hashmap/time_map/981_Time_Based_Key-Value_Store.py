...
timeMap.set("foo", "bar", 1);  // store the key "foo" and value
"bar" along with timestamp = 1.
timeMap.get("foo", 1);         // return "bar"
timeMap.get("foo", 3);         // return "bar", since there is 
no value corresponding to foo at timestamp 3 and timestamp 2, 
then the only value is at timestamp 1 is "bar".
timeMap.set("foo", "bar2", 4); // store the key "foo" and value
"bar2" along with timestamp = 4.
...
# Key Note:
# 1. use map to store timestamp and value as a pair as value
# 2. when we want to get it, we can get timestamp and value
# pairs first, then use binary search to search the value based
# on timestamp
