...
recentCounter.ping(1);     // requests = [1], range is [-2999,1], 
return 1
recentCounter.ping(100);   // requests = [1, 100], range is 
[-2900,100], return 2
recentCounter.ping(3001);  // requests = [1, 100, 3001], range is 
[1,3001], return 3
recentCounter.ping(3002);  // requests = [1, 100, 3001, 3002], 
...
# Key Note:
# 1. this is basically a sliding window problem but we need to know
# how to handle a quque.
# 2. deque = fast, lightweight, perfect for data structure problems
# queue.Queue = safe, slower, used for multithreading or async 
# tasks
# 3. we only need to implement the class function and dont need to
# call it, so return len(self.q) is just fine.
